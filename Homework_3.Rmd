...
demographic_accel_df %>% 
  pivot_longer(
    ...
    names_to = "minute", 
    names_prefix = "min"
  ) %>% ggplot(aes(x = minute, y = data, color = sex)) + geom_point(alpha = 0.3) + geom_smooth() + facet_grid(~education) + theme(axis.text = element_text(angle = 45, vjust = 1, hjust = 1)) + 
  labs(
    title = "Activity by Education Level and Sex", 
    x = "Minute", 
    y = "Activity",
    color = "Sex"
  )
...
The activity seems consistent across education levels. There seem to be low activity during the night and morning, corresponding to sleeping. The activity increases throughout the day. Education level 1 shows denser activity throughout the day, while education level = 3 has higher peaks. Both sexes seem to have similar daily patterns. Sex = 2 has slightly higher activity peaks compared to sex = 1. 
